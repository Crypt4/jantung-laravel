...
class HandleHttpRequestEvent extends Base
{
    ...
    public function handle(RequestHandled $event)
    {
        ...
        if (in_array($status_code, config('jantung.http.ignored_status_codes'))) {
            return;
        }
    ...
    protected function headers($headers)
    {
        ...
        return $this->hideParameters($headers,
            config('jantung.http.hidden_request_headers')
        );
    }
    ...
    protected function payload($payload)
    {
        return $this->hideParameters($payload,
            config('jantung.http.hidden_parameters')
        );
    }
        ...
        if (is_string($content)) {
            if (is_array(json_decode($content, true)) &&
                ...
                return $this->contentWithinLimits($content)
                        ? $this->hideParameters(json_decode($content, true), config('jantung.http.hidden_response_parameters'))
                        : 'Purged By Jantung';
            }

            if (Str::startsWith(strtolower($response->headers->get('Content-Type')), 'text/plain')) {
                return $this->contentWithinLimits($content) ? $content : 'Purged By Jantung';
            }
        }
